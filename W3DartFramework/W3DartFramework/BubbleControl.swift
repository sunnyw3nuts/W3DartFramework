...
//            }
    ...
    func defaultInit () {
        ...
        // events
        addTarget(self, action: #selector(BubbleControl.touchDown), for: UIControl.Event.touchDown)
        addTarget(self, action: #selector(BubbleControl.touchUp), for: UIControl.Event.touchUpInside)
        addTarget(self, action: #selector(BubbleControl.touchDrag(sender:event:)), for: UIControl.Event.touchDragInside)
        ...
        center.x = (mainWindow?.w)! - w/2 + snapOffset
        center.y = 84 + h/2
        snap()
    }
    ...
    @objc func touchDrag (sender: BubbleControl, event: UIEvent) {
        bubbleState = .Drag
        ...
        let location = touch.location(in: mainWindow!)
    ...
    @objc func navButtonPressed (sender: AnyObject) {
    ...
    func popToNavBar () {
        ...
        if image != nil {
            ...
            barButton = UIBarButtonItem (barButtonSystemItem: UIBarButtonItem.SystemItem.action
        ...
        if let last = mainWindow!.rootViewController as? UINavigationController {
    ...
    func popFromNavBar () {
        if let last = mainWindow!.rootViewController as? UINavigationController {
            ...
            self.position = CGPoint(x: mainWindow!.right, y: mainWindow!.top)
    ...
    func openContentView () {
        if let v = contentView {
            ...
            let topvc = mainWindow?.visibleViewController
            currentVC = topvc!
            let img = currentVC.view.layer.makeSnapshot()!
            W3DartVC.shared.setData(imgSS: self.imgSS, createdSS: img)
